...
class MLP(object):
    def __init__(self, layer_sizes, activation_names, lr=0.01):
        ...
        for i in range(len(layer_sizes) - 1):
            self.W.append(np.random.normal(0, 0.01, (layer_sizes[i], layer_sizes[i+1])))
...
def load_omniglot_data(base_path="omniglot_data", img_size=28, max_classes=10, max_samples_per_class=20):
    ...
    for alphabet in os.listdir(dataset_dir):
...
def show_misclassified(X_val, Y_val_labels, predictions, img_size=28, num_images=5):
    ...
    for i in range(num_images):
        ...
        axes[i].set_title(f"True: {Y_val_labels[idx]}, Pred: {predictions[idx]}")
        axes[i].axis('off')
        
    plt.show()

if __name__ == "__main__":
    X, Y, Y_labels, num_classes = load_omniglot_data("omniglot_data", img_size=28, max_classes=10, max_samples_per_class=20)
    ...
    models_configs = [
        {"name": "Base (1 Hid, ReLU, lr=0.1)", "layers": [input_size, 128, output_size], "acts": ['relu'], "lr": 0.1},
        {"name": "Deep (2 Hid, Tanh, lr=0.05)", "layers": [input_size, 128, 64, output_size], "acts": ['tanh', 'tanh'], "lr": 0.05},
        {"name": "Deep (2 Hid, LeakyReLU, lr=0.01)", "layers": [input_size, 128, 64, output_size], "acts": ['leaky_relu', 'leaky_relu'], "lr": 0.01},
        {"name": "Deep (2 Hid, PReLU, lr=0.01)", "layers": [input_size, 128, 64, output_size], "acts": ['prelu', 'prelu'], "lr": 0.01},
        {"name": "Deep (2 Hid, ELU, lr=0.01)", "layers": [input_size, 128, 64, output_size], "acts": ['elu', 'elu'], "lr": 0.01}
    ]
    ...
    for config in models_configs:
        ...
        history = model.train(X_train, Y_train, X_val, Y_val, epochs=epochs_count)
        ...
        print(f"Модель [{config['name']}] успішно навчена.")
